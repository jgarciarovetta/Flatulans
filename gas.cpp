...
void Flatulan::move()
{
    ...
    if(dir == DOWN)
    {
        if(m_row != m_city->rows() && !(m_city->player()->row()==m_row+1 && m_city->player()->col()==m_col))
        {
            m_row++;
        }
    }
    ...
    {
        if(m_row != 1 && !(m_city->player()->row()==m_row-1 && m_city->player()->col()==m_col))
        {
            m_row--;
        }
    }
    ...
    else if (!(m_city->player()->col()==m_col-1 && m_city->player()->row()==m_row))
    {
        if(m_col != 1)
        {
            m_col--;
...
void Player::move(int dir)
{
    ...
    else if (m_city->nFlatulansAt(m_row, m_col+1)==0)
    {
        if(m_col != m_city->cols())
        {
            m_col++;
...
bool City::determineNewPosition(int& r, int& c, int dir) const
{
    ...
    {
        case DOWN:
            if( isInBounds(r+1, c) && nFlatulansAt(r+1, c)==0)
            {
                r++;
            }
            break;
        case UP:
            ...
            break;
        case LEFT:
            if( isInBounds(r, c-1) && nFlatulansAt(r, c-1)==0)
            {
                c--;
            }
            break;
...
void City::display() const
{
    ...
    for (r = 1; r <= rows(); r++){
        for (c = 1; c <= cols(); c++){
            ...
            {
                flatNum = (char)nFlatulansAt(r, c);
                grid[r-1][c-1] = flatNum + '0';
            }
            else{
                grid[r-1][c-1] = '9';
            }
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    for(int i = 0; i<m_nFlatulans; i++)
    {
        fRow=m_flatulans[i]->row();
        fCol=m_flatulans[i]->col();
        
        
        if((fRow==pRow+1 && fCol==pCol) || (fRow==pRow && fCol==pCol+1) || (fRow==pRow-1 && fCol==pCol) || (fRow==pRow && fCol==pCol-1) ||
           (fRow==pRow+1 && fCol==pCol+1) || (fRow==pRow-1 && fCol==pCol+1) || (fRow==pRow+1 && fCol==pCol-1) || (fRow==pRow-1 && fCol==pCol-1))
        {
            //cerr << "working " << endl;
            if(m_flatulans[i]->possiblyGetConverted()){
                //cerr << "deleted " << (m_flatulans[i]==NULL) << endl;
                delete m_flatulans[i];
...
void City::moveFlatulans()
{
    ...
    {
        m_flatulans[k]->move();
        fRow = m_flatulans[k] -> row();
        fCol = m_flatulans[k] -> col();
        if((fRow==pRow+1 && fCol==pCol) || (fRow==pRow && fCol==pCol+1) || (fRow==pRow-1 && fCol==pCol) || (fRow==pRow && fCol==pCol-1))
        {
            m_player->getGassed();
        }
